...
import { HTTPResponseCodes } from "../deps.ts";
import { BASE_URL } from "../deps.ts";
import { RequestMethod } from "../deps.ts";
import { RestManager } from "../deps.ts";
...
//Custom sendRequest Function for more logging
export async function sendRequestCustom<T>(rest: RestManager, options: RestSendRequestOptions): Promise<T> {
  try {

    utils.commonLoggers.logMessage('custom.ts', 'Entering sendRequestCustom');
    // CUSTOM HANDLER FOR USER TO LOG OR WHATEVER WHENEVER A FETCH IS MADE
    rest.fetching(options);

    utils.commonLoggers.logMessage('custom.ts', `Call Url: ${options.url.startsWith(BASE_URL) ? options.url : `${BASE_URL}/v${rest.version}/${options.url}`}`);
    utils.commonLoggers.logMessage('custom.ts', `Method: ${options.method}`);
    utils.commonLoggers.logMessage('custom.ts', `Headers: ${options.payload?.headers}`);
    utils.commonLoggers.logMessage('custom.ts', `Body: ${options.payload?.body}`);
    ...
    utils.commonLoggers.logMessage('custom.ts', `[REST - Request Complete] Payload: ${JSON.stringify(options)} | Response: ${JSON.stringify(response)}`);
    utils.commonLoggers.logMessage('custom.ts', `Response Status: ${JSON.stringify(response.status)}`);
    utils.commonLoggers.logMessage('custom.ts', `Response Status Text: ${JSON.stringify(response.statusText)}`);

    if (response.status < 200 || response.status >= 400) {
      rest.debug(
        `[REST - httpError] Payload: ${JSON.stringify(options)} | Response: ${JSON.stringify(response)}`,
      );
      utils.commonLoggers.logMessage('custom.ts', `[REST - httpError] Payload: ${JSON.stringify(options)} | Response: ${JSON.stringify(response)}`);
      ...
      utils.commonLoggers.logMessage('custom.ts', `Error: ${error}`);
      ...
      if (
        ...
        if (!rest.invalidRequestsTimeoutId) {
          rest.invalidRequestsTimeoutId = setTimeout(() => {
            ...
            utils.commonLoggers.logMessage('custom.ts', `[REST - processGlobalQueue] Resetting invalid optionss counter in setTimeout.`);
      ...
      if (response.status !== 429) {
        ...
        if (options.retryCount && options.retryCount++ >= rest.maxRetryCount) {
          ...
          utils.commonLoggers.logMessage('custom.ts', `[REST - RetriesMaxed] ${JSON.stringify(options)}`);
    ...
    if (response.status === 204) {
      ...
      utils.commonLoggers.logMessage('custom.ts', `[REST - FetchSuccess] URL: ${options.url} | ${JSON.stringify(options)}`);
      options.respond?.({
        ok: true,
        status: 204,
      });
      ...
      utils.commonLoggers.logMessage('custom.ts', `[REST - fetchSuccess] ${JSON.stringify(options)}`);
      options.respond?.({
        ok: true,
        status: 200,
        body: json,
    ...
    utils.commonLoggers.logMessage('custom.ts', `[REST - fetchFailed] Payload: ${JSON.stringify(options)} | Error: ${error}`);
    options.reject?.({
      ok: false,
      status: 599,
